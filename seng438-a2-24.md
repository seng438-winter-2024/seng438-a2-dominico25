...
The testing strategy used to approach the DataUtilities class methods involves partitioning the classes methods and examining the valid and invalid inputs of the defined functions. Methods in this class do not have specific testable boundaries. Instead, they have specified inputs that they are allowed to take in, and defined outputs that depend on these cases. These inputs can be partitioned based on invalid and valid values. An example of this is the calculateColumnTotal() method. This method takes in two arguments, a Values2D object and an integer index. In the documentation for the org.jfree.data library it is specified that this method will throw an InvalidParameterException if an invalid data object is passed in. It also should return 0 if invalid input is entered. These exceptions are what we used to partition and build the test cases for DataUtilities class.

The calculateColumnTotal() and calculateRowTotal() methods are partitioned into test cases using valid and invalid data. Partitioning creates equivalence classes that are used to define our test cases. We approached black-box testing for these functions using test cases that follow the decided equivalence classes. The cases are tested further with specific test cases for Values2D objects with negative values, double type values, and only one value. The invalid data case is tested using a Values2D object filled with null for values, as well as null itself. This structure of test cases applies to both calculateColumnTotal() and calculateRowTotal() as they have the same described function but perform it using different values in the Values2D type. A list of test cases for the calculateColumnTotal() includes “testCalcColumnNullObject()”, “testCalculateColumnTotalForTwoValues()”, 	“testCalculateColTotalForNegativeValues()”, “testCalcColNullDataThrowsException()”, and “testColumnTotalSingleValue()”. The test cases used for calculateRowTotal() are as follows; “testCalcRowNullObject()”, “testCalculateRowTotalWithTwoDoubleValues()”, “testRowTotalWithTwoNegatives()”, “testCalcRowNullDataThrowsException()”, and “testRowTotalSingleValue()”. In depth descriptions for these test cases can be found in the DataUtilities list of test cases in Part 3 of this report. 

The method getCumulativePercentages() is also partitioned using its valid and invalid inputs. Test cases are split into those testing valid inputs such as any KeyedValues object with valid values, and the inputs that should throw exceptions. The test case “testInvalidParameterExceptionThrownWithNullValue()” tests the invalid input by passing in a null object to the method. Other test cases for this function pass in mock data for KeyedValues objects. These thoroughly test the different data types that should calculate the percentage as expected. Some examples of cases that we tested are “testCorrectCumulativePercentageForPositiveIntegers()”, “testCorrectCumulativePercentageForFloatingPointValues()”, “testCorrectCumulativePercentageForNegatives()”, “testCorrectCumulativePercentageForSinglePair()”, and “testCumulativePercentageForEmptyData()”. The tests that cover the different data types in our testing strategy pass KeyedValues objects containing different types of Java Numbers. This ensures thorough testing of the different types that can use this function. Testing with a single pair and an empty set of data tests that the function will perform as expected when these different valid inputs are passed into it. 
...
| 19 | testValidValueForCreateNumberArray2D() | createNumberArray2D() | (data) double[][] = {{1.1, -2.2}, {-3.3, 4.4}} | The return of this function should be an array of type Number | Tesing createNumberArray2D() with valid double values |
| 20 | testEmptyValueForCreateNumberArray2D() | createNumberArray2D() | (data) double[][] = {} | The return of this function should be an empty array of type Number | Tesing createNumberArray2D() with an empty data input |
