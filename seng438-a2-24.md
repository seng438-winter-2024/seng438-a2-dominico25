...
When beginning to write tests for the Data Utilities class it became evident that a different type of testing variable would be necessary, this testing concept would be mocking. Mocking was extremely useful throughout the this assignment, having different data types can cause confusion while testing but using mocks gave the tests that required the additional setup a consistent structure. Having the ability to set the return values of each of the built-in functions of KeyedValues and Values2D gave us much more control to test specific parts of each datatype object to ensure robust testing. The main drawback we found when completing tests that used or required mocking was the setup. As it was everybody's first time using mocking there was a slight learning curve which did take some time, however there is sufficient information on this concept available for us to learn from. For this particlular assignment multiple mock set ups were needed for testing the different functionalities of the system, this was another aspect of this testing style which increased the time it took to complete the lab. Overall we found the use of mocking quite valuable and we felt the benefit of the control it provided us with outweighted the extra time it took to set up.
...
| 10 | testCentralValueShouldBeCorrect()   | getCentralValue()    | --    | -- | 0 | Testing the getCentralValue() function for the set range -1 < value < 1 |
| 11 | testLengthShouldBeCorrect()   | getLength()    | --    | -- | 2 | Testing the getLength() function for the set range -1 < value < 1 |
| 12 | testMethodGetLowerBound()   | getLowerBound()    | --    | -- | -1 | Testing the getLowerBound() function for the set range -1 < value < 1 |
| 13 | testMethodGetUpperBound()   | getUpperBound()    | --    | -- | 1 | Testing the getUpperBound() function for the set range -1 < value < 1 |
...
Our team found this lab to be pretty straight-forward, we all had a pretty good understanding of the testing process from the lectures so building test cases went smoothly. Our challenges began with the use of the new concept of mocking, as none of our team mebers had applied experience in using this for testing. This challenge was overcome quite quickly as there are many resoources that offer in depth explainations of what mocking is and how to use it. Another challenge we faced was finding the right style of test to use, there were some test cases which we knew were important to include however they could not be tested with the simple asserts we were comfortable with. These cases mainly revolved around testing with null values and catching expected exceptions, oftentimes the tests we wrote would run with errors instead of passing or failing. We overcame this by completing some research on different exception catching tests as well as discussing our own ideas that could solve the problem. Our work payed off and we were able to write tests to properly and completely test the invalid inputs of null. The main lesson we learned was that often in testing you may come across a test case you don't know how to write a test for or you may be forced to use a new and unfamiliar concept, but often the skills you learn from working through those difficulties are what improve the quality of your tests.

# 6 Comments/feedback on the lab itself

We found this lab to be well organized and have a good workflow structure, we felt the wokr expectations were clear and had a good understanidng of what was expected during the in lab demo. The supplementary powerpoint offered a good overview for the learning goals of the assignment.
